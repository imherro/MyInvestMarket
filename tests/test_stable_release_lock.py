...
class StableReleaseLockTest(unittest.TestCase):
    ...
    def test_historical_snapshot_score_is_locked(self) -> None:
        ...
        rolling = {
            ...
            "liquidity": {"avg_volume_ratio_5d": 1.04, "avg_volume_ratio_slope_5d": 0.06, "sample_count": 5},
            "mainline": {
        ...
        self.assertEqual(record["risk_adjusted_market_position_score"], 25.43)
        self.assertEqual(record["pre_cap_market_position_score"], 25.43)
        self.assertEqual(record["market_position_score"], 25.43)
        self.assertEqual(record["recommended_equity_position_range"], "20%-40%")
        self.assertEqual(record["position_model"]["trend_multiplier"], 1.02)
        self.assertEqual(record["position_model"]["regime_multiplier"], 0.82)
        self.assertEqual(record["market_regime"], "防守或弱修复")
        self.assertEqual(record["market_regime_code"], "distribution")
        self.assertEqual(record["market_regime_label"], "高位派发")
        self.assertEqual(record["trend_state"], "early_trend")
        self.assertEqual(record["trend_state_label"], "趋势初期")
