...
最后用中文简报：基准交易日、run_id、市场机会分、拥挤惩罚、股票账户仓位分、推荐权益区间、配置状态、五仓配置区间、触发的风险上限、数据质量、API验证结果、commit 或跳过原因。
```
...
- `/api/index.allocation_policy` 输出 `allocation_policy_v1` 五仓配置：核心宽基、主线 ETF、龙头、收益防御、现金替代。
